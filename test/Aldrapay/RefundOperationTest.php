...
class RefundOperationTest extends TestCase {
  ...
  public function test_successRefundRequest() {
  	...
  	$amount = rand(10,40);
  	$trackIdSuccess = 'TRACK-'.substr($this->phpVer,0,strpos($this->phpVer,'-')).'-'.date('YmdHi').'-REFUND-OK';
  ...
  public function test_errorRefundRequest() {
  	...
  	$amount = rand(10,40);
  	$trackIdError = 'TRACK-'.substr($this->phpVer,0,strpos($this->phpVer,'-')).'-'.date('YmdHi').'-REFUND-ERR';
  ...
  protected function runParentTransaction($amount = 10.00, $trackId = null ) {
    ...
    $transaction->setDescription('test payment refund '.substr($this->phpVer,0,strpos($this->phpVer,'-')).'-'.date('YmdHi'));
