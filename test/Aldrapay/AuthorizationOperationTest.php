...
class AuthorizationOperationTest extends TestCase {
  ...
  public function test_buildRequestMessage() {
    ...
    $arr = array(
    		...
    		'description' => 'test auth '.substr(self::getCurrentPhpVer(),0,strpos(self::getCurrentPhpVer(),'-')).'-'.date('YmdHi'),
    );
  ...
  protected function getTestObject($threed = false) {
    ...
    $transaction->setDescription('test auth '.substr(self::getCurrentPhpVer(),0,strpos(self::getCurrentPhpVer(),'-')).'-'.date('YmdHi'));
